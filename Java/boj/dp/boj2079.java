...
public class boj2079 {
    ...
    private static void solve() throws Exception {
        ...
        for(int i=1; i<L; i++) {
            if(str.charAt(i-1) == str.charAt(i)) isP[i-1][i] = true;
        }
        ...
        for(int end=1; end<L; end++) {
            for(int start=1; start<end+1; start++) {
                if(isP[start][end]) dp[end] = Math.min(dp[end], dp[start-1]+1);
                else dp[end] = Math.min(dp[end], dp[end-1]+1);
            }
        }
