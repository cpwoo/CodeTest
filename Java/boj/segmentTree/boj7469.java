...
public class boj7469 {
    ...
    private static void solve() throws Exception {
        ...
        for(int i=0; i<m; i++) {
            ...
            sb.append(tree[1][bisectLeft(a,b,c)-1]).append('\n');
        }
    ...
    private static int bisectLeft(int i, int j, int k) {
        int left = 0, right = n;

        while(left < right) {
            int mid = (left+right)/2;
            if(query(1, 1, n, i, j, tree[1][mid]) <= k-1) left = mid+1;
            else right = mid;
        }
    ...
    private static int bisetRight(int[] arr, int num) {
    ...
    private static int query(int node, int start, int end, int left, int right, int num) {
        ...
        if(left <= start && end <= right) return bisetRight(tree[node], num);
